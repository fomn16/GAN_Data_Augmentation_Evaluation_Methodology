...
class AUGCGAN_MNIST(Augmentator):
    ...
    def createGenModel(self):
        ...
        X = self.AddTransposedResidualBlock(X, 1, 32, stride=2)
        X = self.AddTransposedResidualBlock(X, 1, 32, stride=2)
        ...
        X = self.AddResidualBlock(X, 1, 32)
        X = self.AddResidualBlock(X, 1, 32)
    ...
    def createDiscModel(self):
        ...
        X = self.AddResidualBlock(X, 1, 32, stride=2)
        X = self.AddResidualBlock(X, 1, 32, stride=2)
        ...
        for epoch in range(startEpoch+1, self.ganEpochs):
            ...
            for i in range(nBatches):
                for j in range(self.extraDiscEpochs):
                    ...
                    XImg = np.concatenate((imgBatch, imgBatch))
                    XLabel = np.concatenate((labelBatch, labelBatch))
                    
                    XImg2 = np.concatenate((realImagesShuffled, genImgOutput))
                    XLabel2 = np.concatenate((realLabelsShuffled, labelBatch))
                ...
                if i == nBatches-1:
                    ...
                    showOutputAsImg(out, self.basePath + '/output_f' + str(self.currentFold) + '_e' + str(epoch) + '_' + '_'.join([str(a) for a in labelBatch[:20]]) + '.png')
